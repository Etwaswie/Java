...
public class ParseLine {
    ...
    static void parseLine(String readClientStream, CopyOnWriteArrayList<Ship> ships, Socket clientSocket, Date date){
        ...
        String creator = ab[4].substring(0,ab[4].indexOf("\""));


        if (a.startsWith("add{")) {
            Commands.add(ships, name, size, place, clientSocket,creator);

        } else if (a.startsWith("add_if_max")) {
            Commands.add_if_max(ships, name, size, place, clientSocket,creator);
        } else if (a.startsWith("add_if_min")) {
            Commands.add_if_min(ships, name, size, place, clientSocket,creator);
